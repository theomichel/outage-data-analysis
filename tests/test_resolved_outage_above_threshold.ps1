# Test for Resolved Outage Above Threshold Scenario
# Tests the full pipeline with a large outage that gets resolved and meets thresholds
#
# Parameters:
...
Write-Host "🧪 Resolved Outage Above Threshold E2E Test with Mock Git" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    # Modify git_mock.py to import from test_data_resolved_above_threshold instead of test_data
    $gitMockContent = Get-Content "git_mock.py" -Raw
    $modifiedContent = $gitMockContent -replace 'from test_data import get_default_mock_data as get_test_data', 'from test_data_resolved_above_threshold import get_default_mock_data as get_test_data'
    ...
    if ($createExitCode -eq 0) {
        ...
        # Check for resolved outage notification file - should exist for above threshold scenario
        ...
        if ($notificationFiles.Count -gt 0) {
            ...
            if ($notificationContent -match "2000") {
                Write-Host "+ Notification contains expected customer count (2000)" -ForegroundColor Green
            } else {
        ...
        if ($testsPass -eq $totalTests) {
            Write-Host ""
            Write-Host "Resolved Outage Above Threshold Test PASSED!" -ForegroundColor Green
            Write-Host "The pipeline successfully detected and sent notifications for a resolved outage that met thresholds." -ForegroundColor Green
        } else {
            Write-Host ""
            Write-Host "Resolved Outage Above Threshold Test FAILED!" -ForegroundColor Red
