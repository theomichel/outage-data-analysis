...
try {
    ...
    # Modify git_mock.py to import from test_data_resolved instead of test_data
    $gitMockContent = Get-Content "tests\git_mock.py" -Raw
    $modifiedContent = $gitMockContent -replace 'from test_data import get_default_mock_data as get_test_data', 'from test_data_resolved import get_default_mock_data as get_test_data'
    ...
    $outputDir = "test_resolved_output"
    ...
    $createResult = py create_outages_dataframe.py -d $outputDir -u pse --latestfiles -o "test_resolved_current_outages.csv" 2>&1
    ...
    if ($createExitCode -eq 0) {
        ...
        $analyzeCmd = "py analyze_current_outages.py -u pse -f `"test_resolved_current_outages.csv`" -l 6 -c 100 -e 0.25"
        ...
        # Check for resolved outage notification file
        $notificationFiles = Get-ChildItem -Filter "notification_resolved_*.txt" -ErrorAction SilentlyContinue
        if ($notificationFiles.Count -gt 0) {
            Write-Host "+ Resolved outage notification file(s) created successfully" -ForegroundColor Green
            $testsPass++
            
            # Check notification content
            $notificationContent = Get-Content $notificationFiles[0].FullName -Raw
            if ($notificationContent -match "INC789012") {
                Write-Host "+ Notification contains expected resolved outage ID (INC789012)" -ForegroundColor Green
            } else {
                Write-Host "- Notification missing expected resolved outage ID" -ForegroundColor Yellow
            }
            
            if ($notificationContent -match "RESOLVED OUTAGE ALERT") {
                Write-Host "+ Notification contains expected 'RESOLVED OUTAGE ALERT' text" -ForegroundColor Green
            } else {
                Write-Host "- Notification missing expected 'RESOLVED OUTAGE ALERT' text" -ForegroundColor Yellow
            }
            
            # Check for customer count
            if ($notificationContent -match "1500") {
                Write-Host "+ Notification contains expected customer count (1500)" -ForegroundColor Green
            } else {
                Write-Host "- Notification missing expected customer count" -ForegroundColor Yellow
            }
        } else {
            Write-Host "- No resolved outage notification file created" -ForegroundColor Red
        }
        
        # Check that no new outage notifications were created (since this is a resolved scenario)
        $newNotificationFiles = Get-ChildItem -Filter "notification_new_*.txt" -ErrorAction SilentlyContinue
        ...
        if ($testsPass -eq $totalTests) {
            Write-Host ""
            Write-Host "Resolved Outage E2E Test PASSED!" -ForegroundColor Green
            Write-Host "The pipeline successfully detected and sent notifications for a resolved outage scenario." -ForegroundColor Green
        } else {
            Write-Host ""
            Write-Host "Resolved Outage E2E Test FAILED!" -ForegroundColor Red
            Write-Host "Notification verification failed - there may be issues with the resolved outage detection." -ForegroundColor Red
        }
    } else {
    ...
    if (!$KeepFiles) {
        ...
        # Clean up test files
        Remove-Item -Path "test_resolved_output" -Recurse -Force -ErrorAction SilentlyContinue
        Remove-Item -Path "test_resolved_current_outages.csv" -Force -ErrorAction SilentlyContinue

        Remove-Item -Path "notification_resolved_*.txt" -Force -ErrorAction SilentlyContinue
    } else {
        Write-Host ""
