...
serve(async (req) => {
  try {
    ...
    const config: Config = {
      ...
      enable_zip_filtering: Deno.env.get('ENABLE_ZIP_FILTERING') === 'true',
    };
    ...
    // Get utilities to monitor
    const utilitiesToMonitor: Utility[] = (Deno.env.get('UTILITIES') || 'pse,scl,snopud,pge').split(',') as Utility[];

    const results = [];

    for (const utility of utilitiesToMonitor) {
